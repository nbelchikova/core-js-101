...
function isBracketsBalanced(str) {
  ...
  const close = [']', '}', ')', '>'];
  for (let i = 0; i < str.length; i += 1) {
    if (open.includes(str[i]) === true) {
      result.push(str[i]);
    } else if (close.indexOf(str[i]) === open.indexOf(result[result.length - 1])) {
      result.pop();
    } else {
      return false;
    }
